...
//  import loadFeatures from "@/utils/features"
 const TodoItem: FC<{
   ...
   const [item, setItems] = useState(todosData);
   ...
   const handleDelete = async (id: number) => {
     setIsDeleting(true)
     try {
       const res = await fetch("/api/dele_todo", {
         method: "DELETE",
       ...
       {item.map((Todo) => (
         ...
         >
           <div className="flex items-center space-x-5">
             <div className="flex-none w-3 h-3 rounded-full focus:bg-red-300 bg-primary"></div>
             <p className="text-lg font-normal text-slate-800">{Todo.task}</p>
           </div>
           {todosData.length > 3 &&
